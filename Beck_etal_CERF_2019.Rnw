...
\vspace{-0.2in}
\begin{center}
\animategraphics[controls,width=\linewidth]{12}{fig/anidoy2}{}{} %frame rate is 12 per/sec
\end{center}
\end{frame}
...
\vspace{-0.15in}
\begin{center}
\includegraphics[width=\textwidth]{fig/alldoy2.pdf}
\end{center}
\end{frame}
...
latex(totab, file = '', rowlabel = 'Model', size = 'tiny', rowname = NULL, cgroup = c('', 'Stations (north to south)'), 
      n.cgroup = c(1, 9), caption = "Generalized cross-validation (GCV) summary statistics by station and model")
@
